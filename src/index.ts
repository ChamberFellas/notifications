...
const pollNotifs = async () => {
  ...
  if (chores && bills) {
    const incompleteChores = chores.filter(
      (chore: Chore) => chore.status == "incomplete",
    );
...
const testPostChore = async () => {
  ...
  try {
    const response = await axios.post(`http://${HOST}:${PORT}/chores`, {
      chore,
    });
